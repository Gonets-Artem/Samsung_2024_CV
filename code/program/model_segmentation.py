...
class ModelSegmentation:
    ...
    def predict(self, start_image, size_model_frame=(512,512), size_display=(480,640)):
        img = cv2.cvtColor(start_image, cv2.COLOR_BGR2RGB) #/ 255
        img = A.Compose([A.Resize(size_model_frame[0], size_model_frame[1]),])(image=img)['image'] / 255  #
        ...
        pred = functional.resize(img=pred, size=(size_display[0],size_display[1]))  #
